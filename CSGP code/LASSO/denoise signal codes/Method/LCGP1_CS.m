function [x,x_debias,objective,times,debias_start,mses,taus]= ...
    LCGP1_CS(y,A,tau,varargin)
%
% A hybrid three-term conjugate gradient projection method for constrained nonlinear monotone equations with applications(2021)
%
...
%          dv = - Lv + Betak*old_dv + lambdak*ykv;
         % compute y_{k-1}=(yku,ykv), s_{k-1}=(sku,skv), 
         sku = u-old_u; % x=(u,v), old_x=(old_u,old_v)
         skv = v-old_v;
         yku = Lu-old_Lu;  % Fk=(Lu,Lv); F_{k-1}=(Lu, Lv)
         ykv = Lv-old_Lv;
         
%          rk=1+max(0,-([yku(:);ykv(:)]'*[du(:);dv(:)])/norm([old_Lu(:);old_Lv(:)])^2);
%          gammaku=yku+rk*du;
%          gammakv=ykv+rk*dv;
         
         % convex coefficience
         sk_hatu = sku + (1+max(0,-[sku(:);skv(:)]'*[yku(:);ykv(:)]/norm([yku(:);ykv(:)])^2))*yku;
         sk_hatv = skv + (1+max(0,-[sku(:);skv(:)]'*[yku(:);ykv(:)]/norm([yku(:);ykv(:)])^2))*ykv;
         tauk=norm([yku(:);ykv(:)])^2/([yku(:);ykv(:)]'*[sk_hatu(:);sk_hatv(:)]);
         %tauk=1/(exp(iter+1)^(iter+1));
         ...
         % DDPM is not good
         %thetak1=([gammaku(:);gammakv(:)]'*[sku(:);skv(:)])/(norm([gammaku(:);gammakv(:)])^2);
         
         % MSP
         thetak1=([sku(:)+lambdak*yku(:);skv(:)+lambdak*ykv(:)]'*[yku(:);ykv(:)])/norm([yku(:);ykv(:)])^2;
         
         % PSGM
         thetak2=norm([sku(:);skv(:)])/norm([yku(:)+lambda*sku(:);ykv(:)+lambdak*skv(:)]);
         %thetak2=norm([sku(:);skv(:)])^2/([yku(:)+lambdak*sku(:);ykv(:)+lambdak*skv(:)]'*[sku(:);skv(:)]);
         
         %tauk=1;
         thetak=min(1e30,max((1-tauk)*thetak1+tauk*thetak2,1e-30));
         % computation of search direction vector
         du = -thetak*Lu;
         dv = -thetak*Lv;
...
%       old_gradv = gradv;
      ...
      % line search  process
      while - Luvduv < sigma*betas*dudv*NormFz % - Luvduv < sigma*betas*max(0.001,min(0.8,NormFz))*dudv 
...
%       NormFz2=Lu(:)'*Lu(:)+Lv(:)'*Lv(:);
      ...
      lambda = -1.3*Luvduv*betas/(Lu(:)'*Lu(:)+Lv(:)'*Lv(:));  % lambda = - 1.6*Luvduv*betas/NormFz^2;
      %
       u = old_u - lambda * Lu;
       v = old_v - lambda * Lv;
