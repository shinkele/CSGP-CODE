...
fid=fopen('result/data_repeats_result.txt','w');
...
for index=8
    ...
    % n_spikes = floor(.01*n);
    n_spikes = 32*index;  % number of nonzeros 128
    % generate problems
    for repeats=1:20
        % random +/- 1 signal
        f = zeros(n,1);
...
% %         end
        ...
        fprintf(fid,'%d & %d/%.2f/%.3e & %d/%.2f/%.3e & %d/%.2f/%.3e%s\n',repeats,message,' \\');
    end
    fprintf(fid,'%s & %.2f/%.2f/%.3e & %.2f/%.2f/%.3e & %.2f/%.2f/%.3e%s\n','Average',mean(message,1),' \\'); 
