 ...
 function [Tcpu,NF,Itr,NG] = HSDY_CS(n,x0,nprob) 
format long
...
%                     end
            ...
            case Step(3) % 投影
                ...
                zk1=x0 - 1.2 * xik * Fzk;     % 此处通常的投影法系数是1
...
%                 end
                  yk = Fk - Fk0;sk = x1 - x0;
                  sk_hat=sk+max(0,-sk'*yk/norm(yk)^2)*yk+yk;
                  thetak=norm(yk)^2/(yk'*sk_hat);
                  
                  tk=1+max(0,-dk'*yk/norm(dk)^2);
                  wk=yk+tk*dk;
                  betak_mhs=Fk'*yk/(dk'*wk);
                  betak_mdy=norm(Fk)^2/(dk'*wk);
                  betak_hsdy=(1-thetak)*betak_mhs+thetak*betak_mdy;
                  dk = - (1-betak_hsdy*Fk'*dk/norm(Fk)^2)* Fk+betak_hsdy*dk;
                %==========================================================
                x0=x1; 
                if norm(dk)<epsilon1
                    nextstep=Step(5);
